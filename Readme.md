...
Dự án này xây dựng một giải pháp Deep Learning toàn diện để dự báo xu hướng giá của các cổ phiếu Blue-chip (nhóm VN30) tại Việt Nam. Thay vì chỉ sử dụng dữ liệu giá đóng cửa đơn thuần, hệ thống tích hợp các **Chỉ báo Phân tích Kỹ thuật (RSI, MACD, SMA/EMA )** vào kiến trúc mạng **Stacked LSTM (Long Short-Term Memory)** để học được các mẫu hình biến động phức tạp của thị trường.
...
| **MWG**  | 7.16 |       11.15%       | Bám sát xu hướng nhưng có độ trễ ở một số nhịp biến động mạnh.      |
...
4. **Huấn luyện:** Tập dữ liệu được phân chia theo thời gian thành 80% Train và 20% Test. Trong quá trình học trên tập Train, 10% dữ liệu được trích ra làm tập kiểm định (Validation set) kết hợp cùng Early Stopping để tránh Overfitting.
5. **Đánh giá:** Dự đoán trên tập Test (20% dữ liệu cuối) và so sánh với giá thực tế sau khi giải chuẩn hóa (inverse-scaling).
